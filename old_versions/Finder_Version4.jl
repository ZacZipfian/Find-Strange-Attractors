...
function NewEra5(u, p, n) # here `n` is "time", but we don't use it. no data for this
    x, y = u # system state
    a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11 = p # system parameters
    xn = a0 + a1*sharp(x) + a2*sharp(y) + a3*sharp(x)*sharp(y) + a4*sharp(x^2) + a5*sharp(y^2)
    yn = a6 + a7*sharp(x)  + a8*sharp(y)  + a9*sharp(x)*sharp(y) + a10*sharp(x^2) + a11*sharp(y^2)
    return SVector(xn, yn)
end

attractor_equation = NewEra5
attractor_name = "NewEra5"
complete_time = 20_000
...
chaotic_attractors_save12_V2(complete_time, dataname, 12, attractor_equation)
...
# Generate the Images
generate_stra_atr_series(chao_data, attractor_equation, attractor_choices1, name_val*"_"*iter_val, "ltpd")
