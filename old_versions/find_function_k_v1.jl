...
function NewEra2(u, p, n) # here `n` is "time", but we don't use it. no data for this
    x, y = u # system state
    a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11 = p # system parameters
    xn = a0 + a1*x*sin(y) + a2*y*sin(y) + a3*x*y*sin(y) + a4*sin(x*y) + a5*sin(y)
    yn = a6 + a7*x*sin(x)  + a8*y*sin(x)  + a9*x*y*sin(x) + a10*sin(x*y) + a11*sin(x)
    return SVector(xn, yn)
end

print("Creating CSV file. \n")
# Search over 100k to find new values
chaotic_attractors_save12_V2(100_000, "NewEra2_5", 12, NewEra2)

# Read in new CSV DataFrame
chao_data = CSV.read("NewEra2_5.csv", DataFrame)
...
for i in 1:parse(Int64,input1)
    try
        ...
        henonk = DeterministicIteratedMap(NewEra2, u0k, p0k) #change [1]
...
# Generate the Images
generate_stra_atr_series(chao_data, NewEra2, attractor_choices1, "(kir)", "ltpd")

print("LTPD images done, now KLD images")
generate_stra_atr_series(chao_data, NewEra2, attractor_choices2, "(kir)", "kld")
